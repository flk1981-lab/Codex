...
def add_page_break_paragraph_before(anchor):
...
def main() -> int:
    if len(sys.argv) != 3:
        print("Usage: python generate_final_thesis_docx.py <source.docx> <target.docx>")
        return 1
    ...
    for i, p in enumerate(doc.paragraphs):
        if p.text.strip().replace("\u3000", " ") == "致谢":
            ack_idx = i
            break
    if ack_idx is None:
        print("Could not find 致谢 section.")
        return 1

    ack_anchor = doc.paragraphs[ack_idx]
    insert_block_before(
        ack_anchor,
        [("pagebreak", ""), *COMMITTEE_PLACEHOLDER, ("pagebreak", ""), *ACHIEVEMENTS],
    )
    doc.save(str(target))
    doc = Document(str(target))
    doc.add_page_break()
    append_block(doc, RESUME)
    doc.save(str(target))
    print(f"Generated final draft: {target}")
    return 0
